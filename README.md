...
- The dataset can be found here on the Kaggle website: https://www.kaggle.com/datasets/kartik2112/fraud-detection/code. However, it is important to note that the data was artificially generated using a synthetic data generation tool called Sparkov.
- Please note that the following information should be read along with the Python code in this repository and requires a basic understanding of statistical concepts and the decision trees algorithm. Each concept described in this documentation is explained in more detail in the Concept.md file
...
- This dataset contains each account holder's date of birth, but I think it makes more sense to convert it to their actual age. Another conversion is needed for the date and time of the transaction so that the individual components (year, month, day, and hour) are converted into separate features so that the model can detect patterns in the underlying data.
...
- In this case, most data points are spread between the range of 300 to 900, but being closer to the first quartile. It becomes apparent that there is definetly a connection between the amount spent and fraudulent activities since the median of both target splits have a significant distance inbetween
- Next, let's take a look at the distribution of the jobs that are the most present in the fraudulent-activities split. Note that I also included the proportion of jobs in non-fraudulent activities, since a job might have a general high appearance in the dataset, making it also more likely to be present in fraud transactions, no matter the correlation.
...
- The second part of the preprocessing is used to prepare the data for the model. I removed all irrelevant features and also excluded all personal information such as home address, name, etc. to protect the identity of the people involved (even if they were artificially created).
...
- An important aspect of choosing a model is to make sure that the target class of the dataset is balanced.
