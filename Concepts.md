...
- Correlation values are bound between values of -1 and +1. The bigger the number, the more positive the relationship. The smaller the number, the more negative the relationship. A value close to 0 signals no relationship at all
...
  $$r_p = \frac{\sum(x_i -\bar x)(y_i - \bar y)} {\sqrt{\sum(x_i - \bar x)²\sum(y_i - \bar y)²}}$$
 
...
    
- The first term of the formula, $y_i\log(p)$ measures the loss when y is 1, and the second term, $(1-y_i)\log(1-p)$ measures the loss when y is 0. The negative sign ensures that the overall loss is positive. But instead of assuming $\gamma$ to be the predicted probability *p*, the log of odds is used to make the computation easier. The log of odds is defined to be as
    
...
    $$=-\sum_{i=1}^ny_i+np$$
    
- To take the derivative of the loss functions, simply use the chain rule (the derivative of the nesting expression times the derivative of the nested expression). Note that the fraction is actually defined to be *p* so it can be replaced to arrive at the final solution
...
- This is done by taking the derivative of the loss function with respect to the previous prediction, $F_{m-1}$, and multiplying it by -1. This results in what is known as the residuals

$$r_{i,m}=-\Big[\frac{\delta L(y_i, F(x_i))}{\delta F(x_i)}\Big ]_{F(x)=F_{m-1}(x)}$$
    
- The residuals are computed for each single sample *i*. The gradient provides guidance on the directions (+/-) and magnitude with which the loss function can be minimized by altering the prediction. Now, substituting for the actual loss function:
    
...
    $$=y_i-p$$
    
- Again, the derivative results in the difference between the target variable and the log of odds, which is why *r* is called the residuals
- Now, a regression tree is used with all *x* features to predict the residual. The training data can be run down the first tree, and the resulting prediction gets added to the previous prediction. The sum of these two values results in the new predicted target variable
...
- Training the regression tree results in $R_{j, m}$ for $j=1, ...,J_m$, where *R* is the subset of samples that are assigned to (or predicted to be) at *j (the* terminal node; i.e., a leaf in the tree) at tree index *m*. *J* is the total number of leaves
...
- The second-order Taylor polynomial is used to approximate a function around a given point (in this case, the current model’s predictions) using its first and second derivatives. This simplifies the computation done, and the loss function is now being replaced by an approximation of it
    
...
# Resampling
## Tomek Links
- Tomek links help reduce the imbalance by identifying pairs of samples from different classes that are very close to each other in feature space and then removing the sample from the majority class in each pair

  ![tomek](https://user-images.githubusercontent.com/127037803/224483385-391c65b4-12d1-4fc9-85ec-4ce41a8f0881.png)

- As shown in this graph, the algorithm works by going through each sample of one class, finding the nearest neighbor that is of the opposite class, and repeating the process for the other class. Then removing the sample from the majority class
  

## SMOTE

Synthetic Minority Oversampling Technique (SMOTE) works by randomly picking a point from the minority class and computing the k-nearest neighbors for this point. Then, synthetic points are added between the chosen point and its neighbors
    
...
- In binary classification, the model makes a prediction for each data point, assigning it to 0 or 1. For each possible threshold value between 0 and 1, the model calculates the true positive rate (TPR) and the false positive rate (FPR)
    
